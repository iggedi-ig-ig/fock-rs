...
pub fn hermite_expansion_coeffs(i: i32, j: i32, t: i32, dist: f64, a: f64, b: f64) -> f64 {
    let p = a + b;
    let q = a * b / p;
    ...
    if t < 0 || t > i + j {
        ...
        (2.0 * p).recip() * hermite_expansion_coeffs(i - 1, j, t - 1, dist, a, b)
            - (q * dist / a) * hermite_expansion_coeffs(i - 1, j, t, dist, a, b)
            + (t + 1) as f64 * hermite_expansion_coeffs(i - 1, j, t + 1, dist, a, b)
    } else {
        (2.0 * p).recip() * hermite_expansion_coeffs(i, j - 1, t - 1, dist, a, b)
            + (q * dist / b) * hermite_expansion_coeffs(i, j - 1, t, dist, a, b)
            + (t + 1) as f64 * hermite_expansion_coeffs(i, j - 1, t + 1, dist, a, b)
    }
}
